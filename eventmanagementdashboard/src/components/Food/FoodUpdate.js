...
function FoodUpdate() {
  ...
  const id = window.sessionStorage.getItem("FoodsID");

  useEffect(() => {
    axios.get(`http://localhost:3000/FoodManagement/${id}`).then((response) => {
  ...
  function UpdateCleaningCompany() {
    ...
    //   });
    axios
      .patch(`http://localhost:3000/FoodManagement/${id}`, {
        Name: name,
        Description: Description,
        Quantity: Quantity,
      })
                  ...
                  <Form>
                    ...
                    <Form.Group as={Row} className="mb-3" controlId="">
                      ...
                      <Col sm={9}>
                        <Form.Control
                          type="text"
                          defaultValue={foods.Description}

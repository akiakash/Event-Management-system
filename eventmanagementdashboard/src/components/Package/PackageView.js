...
function PackageView() {
  ...
  const getRequest = () => {
    axios.get("http://localhost:3000/PackageManagement").then((response) => {
  ...
  function deleteCleaning(_id) {
    swal({
      ...
      if (willDelete) {
        ...
        fetch(`http://localhost:3000/PackageManagement/${_id}`, {
          method: "DELETE",
        })
          .then((response) => {
            response.json();
